        ...
        <tr>
            ...
            <td>
                <div style="display: flex; align-items: center; gap: 12px;">
                    <img src="${product.image || placeholderImg}" style="width: 48px; height: 48px; object-fit: cover; border-radius: 8px; border: 1px solid #f0f0f0;" onerror="this.src='${placeholderImg}'">
                    <div>
            ...
            <td style="font-weight: 600; color: ${product.card_available_stock > 0 ? '#52c41a' : '#ff4d4f'};">
                ${product.card_available_stock || 0}
            </td>
            <td>
...
async function loadProductForEdit(id) {
    try {
        ...
        document.getElementById('product-price').setAttribute('data-original-price', productData.price || '0');
...
async function saveProduct() {
    ...
    try {
        ...
        if (window.currentEditProductId) {
            const originalPrice = parseFloat(document.getElementById('product-price').getAttribute('data-original-price') || '0');
            const newPrice = parseFloat(formData.price);
            if (originalPrice > 0 && originalPrice !== newPrice) {
                const confirmed = confirm(`⚠️ 价格变更警告\n\n原价格：¥${originalPrice}\n新价格：¥${newPrice}\n\n价格变更后，现有的引流链接将失效！\n\n是否继续？`);
                if (!confirmed) {
                    return;
                }
            }
